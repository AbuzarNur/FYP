...
for i = 1:num_bpm_samples
    ...
    green_hann = signal_cutoff .* hann(size(signal_cutoff, 2))';
    green_DFT = abs(fft(green_hann));
    
    lower = floor(bpm_range(1) * (size(green_hann,2)/fps)) + 1;
    upper = ceil(bpm_range(2) * (size(green_hann,2)/fps)) + 1;
    bounds = lower:upper;
    
    [peak, location] = findpeaks(double(green_DFT(bounds)));
    [max_peak, max_location] = max(peak);
    max_index = bounds(location(max_location));
    bpm(i) = (max_index-1) * (60*fps/size(green_hann,2));
end
