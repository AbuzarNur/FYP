...
y_centre = round(y_centre - bbox(4)*0.32);
...
points = detectMinEigenFeatures(rgb2gray(videoFrame), 'ROI', ROI,'MinQuality',0.001);
...
grid on;
title('Raw Green Channel');
xlabel('Video Frames'); ylabel('Average Green Value');
%%
% Filtering
fps = 30;
offset = 5;
...
for j = 1:num_bpm_samples
    ...
    title('Frequency Analysis: Heart Rate Peak');
    ylabel('Amplitude');
...
figure;
plot(time, bpm, 'c--','LineWidth', 1.1,'HandleVisibility','off');
title('RG Heartrate');
xlabel('Time (s)');
...
for i = 1:num_bpm_samples
    if abs(bpm(i) - med_bpm) > 0.15*med_bpm
        bpm_final(i) = med_bpm;
%         if bpm_final(i) == bpm_final(i-10)
%             bpm_final(i) = mean(bpm_final);
%         end
    end 
end

average_bpm = mean(bpm_final);
plot(time, bpm_final, 'r','LineWidth', 1.5);
